...
const requiredFiles = [
  'index.html', 'styles.css', 'ui-enhancements.css', 'ui-enhancements.js', 'layer-order.css', 'layer-order.js',
  'functional-ui.js', 'app.js', 'electron-main.js', 'preload.js', 'updater-renderer.js', 'package.json', 'README.md', 'TEMPLATE.md'
];
for (const file of requiredFiles) if (!fs.existsSync(path.join(root, file))) throw new Error(`Missing required file: ${file}`);
for (const file of ['app.js','ui-enhancements.js','functional-ui.js','layer-order.js','electron-main.js','preload.js','updater-renderer.js']) execFileSync(process.execPath, ['--check', path.join(root, file)], { stdio: 'pipe' });
...
if (!packageJson.build.files.includes('layer-order.js') || !packageJson.build.files.includes('layer-order.css')) throw new Error('Layer-order assets are not packaged.');
...
for (const script of ['app.js','layer-order.js','ui-enhancements.js','updater-renderer.js','functional-ui.js']) if (!html.includes(`src="${script}"`)) throw new Error(`Missing UI script: ${script}`);
...
for (const token of ['toDataURL','jpegData','chooseOutputDirectory','frontOverlay','backOverlay','photoState','safeCode','roundedRectPath','drawInfoCard','saveJpgs']) if (!app.includes(token)) throw new Error(`Missing application capability: ${token}`);
if (!app.includes('FileReader')) throw new Error('Photo thumbnail persistence is not implemented.');

const layer = fs.readFileSync(path.join(root, 'layer-order.js'), 'utf8');
for (const token of ['TEXT','photo-front','front-photo','localStorage','drawPhoto','drawText','state.mode']) if (!layer.includes(token)) throw new Error(`Missing layer-order capability: ${token}`);
if (!layer.includes("state.mode === 'photo-front'")) throw new Error('Photo-above-front-artwork mode is not implemented.');
if (!layer.includes("state.mode === 'photo-front' ? 'TEXT  ›  PHOTO  ›  FRONT IMAGE' : 'TEXT  ›  FRONT IMAGE  ›  PHOTO'")) throw new Error('Layer-order status text is incorrect.');

const functional = fs.readFileSync(path.join(root, 'functional-ui.js'), 'utf8');
for (const token of ['openCrop','bakeCrop','preview','downloadJpgs','printCards','saveTemplate','templates','setInputFile','DataTransfer']) if (!functional.includes(token)) throw new Error(`Missing functional UI capability: ${token}`);
...
console.log(`Smoke check passed: ${requiredFiles.length} required files, JavaScript syntax, UI wiring, front layer ordering, generation, crop, preview, download, print, templates, thumbnail persistence, CI Node 24 migration, and desktop capabilities verified.`);
