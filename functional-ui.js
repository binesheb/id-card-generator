(() => {
  ...
  // Front/back artwork: keep the attractive design controls, but make the visible cards
  // real file inputs even though the actual input elements live in the advanced area.
  document.querySelectorAll('.design-upload[for]').forEach(label => {
    label.addEventListener('click', () => { const input=$(label.htmlFor); input?.click(); }, true);
  });
  ...
  // Photo crop/resize. The modal edits the source image by baking the selected crop into
  // a new image, then sends that image through the application's normal photo pipeline.
  ...
  function drawCrop(){
    ...
    ctx.drawImage(sourceImage,(W-iw)/2+cropState.x,(H-ih)/2+cropState.y,iw,ih); ctx.restore();
    ctx.strokeStyle='rgba(8,105,220,.9)'; ctx.lineWidth=3; ctx.strokeRect(frame.x,frame.y,frame.w,frame.h);
    ctx.strokeStyle='rgba(255,255,255,.75)'; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(W/2,frame.y);ctx.lineTo(W/2,frame.y+frame.h);ctx.moveTo(frame.x,H/2);ctx.lineTo(frame.x+frame.w,H/2);ctx.stroke();
  }
  ...
  function bakeCrop(){
    ...
    const W=1000,H=Math.round(W*(TEMPLATE_RATIO));
    const pad=0, fit=Math.max(W/sourceImage.naturalWidth,H/sourceImage.naturalHeight)*cropState.scale;
    const iw=sourceImage.naturalWidth*fit,ih=sourceImage.naturalHeight*fit;
    const previewW=cropCanvas.width-48,previewH=cropCanvas.height-48;
    ...
    ctx.drawImage(sourceImage,(W-iw)/2+px,(H-ih)/2+py,iw,ih);
  ...
  $('closeCrop')?.addEventListener('click',closeCrop,true);
  $('cropDone')?.addEventListener('click',bakeCrop,true);
  ...
  // Preview: open a real modal containing the current canvases instead of merely toggling a CSS class.
  function preview(){
    const existing=$('functionalPreview'); if(existing){existing.remove();return;}
    const modal=document.createElement('div'); modal.id='functionalPreview'; modal.className='functional-modal';
    ...
    [['frontCanvas','previewFront'],['backCanvas','previewBack']].forEach(([a,b])=>{const src=$(a),dst=$(b);dst.width=src.width;dst.height=src.height;dst.getContext('2d').drawImage(src,0,0);});
    modal.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>modal.remove()); modal.querySelector('#previewPrint').onclick=()=>printCards();
  }
  function printCards(){
    const f=$('frontCanvas'),b=$('backCanvas'); const w=window.open('','_blank','width=1200,height=900'); if(!w){alert('Please allow pop-ups for printing.');return;}
    const fu=f.toDataURL('image/jpeg',1),bu=b.toDataURL('image/jpeg',1);
    w.document.write(`<html><head><title>Jayalakshmi ID Card</title><style>@page{size:auto;margin:0}body{margin:0;padding:12mm;display:flex;gap:10mm;justify-content:center;align-items:flex-start}img{width:54mm;height:86mm;object-fit:fill;break-inside:avoid}</style></head><body><img src="${fu}"><img src="${bu}"></body></html>`);w.document.close();w.onload=()=>{w.focus();w.print();};
  }
  ...
  // Templates are persistent and include employee data plus artwork. This makes Save Template
  // and Templates useful without requiring a server.
  const TEMPLATE_KEY='jayalakshmi-id-card-templates-v1';
  async function saveTemplate(){
    const templates=JSON.parse(localStorage.getItem(TEMPLATE_KEY)||'{}');
    const name=prompt('Template name', $('employeeCode')?.value.trim()||'Jayalakshmi Template'); if(!name)return;
  ...
  function templates(){
    const all=JSON.parse(localStorage.getItem(TEMPLATE_KEY)||'{}'); const names=Object.keys(all);
    const choice=prompt(names.length?`Templates:\n${names.map((n,i)=>`${i+1}. ${n}`).join('\n')}\n\nEnter a template name to load:`:'No saved templates yet. Enter a name to save a new template:');
  ...
  // Intercept the placeholder buttons from the visual shell and connect them to real features.
